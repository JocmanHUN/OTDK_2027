...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run all models for daily fixtures")
    p.add_argument(
    ...
    p.add_argument("--json", action="store_true", help="Output JSON array for UI consumption")
    return p


def _serialize_prediction(pred: Any) -> Mapping[str, Any]:
    # pred is a pydantic model (Prediction)
    base = {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # 1) List supported leagues and daily fixtures
    ...
    if not fixtures:
        print("[]" if args.json else "No fixtures to predict.")
        return 0

    # 2) Prepare services and models
    ...
    for f in fixtures:
        ...
        # Build context from API averages
        ...
        # Minimal match; names not needed by models
        match = Match(
